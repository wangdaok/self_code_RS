...
dataset = nc.Dataset("clmforc.0.1x0.1.solar-2010-01.nc")
print(dataset)
print('---------------------------------------')
...
#查看变量的属性
print(dataset.variables['FSDS'])
print(dataset.variables['FSDS'].ncattrs())

#读取变量数据值
datasal = dataset.variables['FSDS'][:].data
print(datasal)
...
#复制与替换
toexclude = ['FSDS', 'time'] #'FSDS', 
dst= nc.Dataset("newclmforc.0.1x0.1.solar-2010-01.nc", "w") 
...
new_var = dst.createVariable('FSDS','f4', dimensions=('time','lat','lon'))
print(type(new_var))
dst['FSDS'].setncatts(dataset['FSDS'].__dict__)

new_var[:]=newdata[:]
print(new_var[0][0][0])


print(dst['FSDS'].__dict__)
nprec=dst.variables['FSDS'][:].data
